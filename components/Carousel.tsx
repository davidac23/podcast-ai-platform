    ...
    <section className="flex w-full flex-col gap-4 overflow-hidden" ref={emblaRef}>
      <div className='flex'>
        {slides.slice(0, 5).map((item) => (
          <figure className='carousel_box' key={item._id} onClick={() => router.push(`/podcasts/${item.podcast[0].podcastId}`)}>
            <Image src={item.imageUrl} fill alt='card' className='absolute size-full rounded-xl border-none' />
            <div className='glassmorhpism-black relative z-10 flex flex-col rounded-b-xl p-4'>
